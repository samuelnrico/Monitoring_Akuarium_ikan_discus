...
def evaluasi_kondisi(suhu, ph, turb):
    ...
    # ---------------- Turbidity ----------------
    if turb > TURB_BAHAYA:               # > 35 -> BAHAYA (tak ada aktuator)
        penyebab.append("Turbidity melewati batas")
        rank = max(rank, 2)
    elif turb > TURB_AMAN_MAX:            # > 25 sampai 35 -> WASPADA
        penyebab.append("Turbidity mulai meningkat")
        rank = max(rank, 1)
...
def terima_data():
    ...
    # ESP32 hanya menutup timer Waspada bila nilai ini True.
    ...
    if status == "Bahaya":
        ...
        telegram_terkirim = kirim_notif_kondisi(
            "Waspada", suhu, ph, turb, penyebab, None
        )

    # 8 — Notifikasi aktuator (ON setiap perintah/dosing; OFF saat normal)
