...
PH_WASPADA_ASAM_MAX = 5.9  # Waspada asam berakhir tepat di 5.9
PH_WASPADA_BASA_MIN = 7.1  # Waspada basa dimulai tepat dari 7.1
...
SUHU_WASPADA_DINGIN_MAX = 24.9  # Waspada dingin sampai tepat 24.9
SUHU_WASPADA_PANAS_MIN  = 31.1  # Waspada panas mulai tepat 31.1
...
# Pemicu khusus untuk setiap notifikasi aktuator otomatis.
PEMICU_AKTUATOR = {
    "pompa_asam": "pH terlalu basa",
    "pompa_basa": "pH terlalu asam",
    "heater":     "Suhu terlalu dingin",
}
...
def format_penyebab_notif(penyebab):
    # Satu parameter ditulis pada baris yang sama tanpa bullet dan label level.
    # Beberapa parameter sudah diformat sebagai daftar oleh evaluasi_kondisi().
    if "\n" in penyebab:
        return "Penyebab  :\n" + penyebab
    return f"Penyebab  : {penyebab}"
...
def pesan_kondisi_waspada(suhu, ph, turb, penyebab):
    return (
        ...
        f"{format_penyebab_notif(penyebab)}"
    )
...
def evaluasi_kondisi(suhu, ph, turb):
    ...
    if ph > PH_BAHAYA_BASA:              # > 7.5 -> Bahaya
        ...
        rank = max(rank, 2)
    elif PH_WASPADA_BASA_MIN <= ph <= PH_BAHAYA_BASA:  # 7.1-7.5 -> Waspada
        masalah_waspada.append("pH cenderung basa")
        rank = max(rank, 1)
    elif PH_BAHAYA_ASAM <= ph <= PH_WASPADA_ASAM_MAX:  # 5.5-5.9 -> Waspada
    ...
    if suhu > SUHU_BAHAYA_PANAS:         # > 33 -> Bahaya, tanpa cooler
        ...
        aksi.append("Heater")
        rank = max(rank, 2)
    elif SUHU_WASPADA_PANAS_MIN <= suhu <= SUHU_BAHAYA_PANAS:  # 31.1-33 -> Waspada
        masalah_waspada.append("Suhu tinggi")
        rank = max(rank, 1)
    elif SUHU_BAHAYA_DINGIN <= suhu <= SUHU_WASPADA_DINGIN_MAX:  # 20-24.9 -> Waspada, heater OFF
    ...
    semua_masalah = (
        [(item, "Bahaya") for item in masalah_bahaya] +
        [(item, "Waspada") for item in masalah_waspada]
    )

    if not semua_masalah:
        penyebab_str = "Semua parameter normal"
    elif len(semua_masalah) == 1:
        # Status utama sudah terlihat, jadi tidak perlu bullet atau label level.
        penyebab_str = semua_masalah[0][0]
    else:
        level_terdeteksi = {level for _, level in semua_masalah}
        campuran_level = len(level_terdeteksi) > 1
        if campuran_level:
            rincian = [
                f"- {item} ({level})" for item, level in semua_masalah
            ]
        else:
            rincian = [f"- {item}" for item, _ in semua_masalah]
        penyebab_str = "\n".join(rincian)
...
def proses_notif_aktuator(aktuator_baru):
    for key, val in aktuator_baru.items():
        ...
        if val:
            kirim_telegram_pesan(
                pesan_aktuator_on(NAMA_AKTUATOR[key], PEMICU_AKTUATOR[key])
            )
...
def aktuator_manual():
    ...
    nama_valid = set(NAMA_AKTUATOR.values())

    if nama not in nama_valid:
        return jsonify({"status": "error", "msg": "nama aktuator tidak valid"}), 400
    if not isinstance(state, bool):
        return jsonify({"status": "error", "msg": "state harus boolean"}), 400
    ...
    telegram_terkirim = kirim_telegram_pesan(pesan)
    logger.info(
        f"[Manual] {nama} -> {'ON' if state else 'OFF'} | "
        f"telegram={telegram_terkirim}"
    )

    return jsonify({
        "status": "ok" if telegram_terkirim else "error",
        "telegram_terkirim": telegram_terkirim,
        "aktuator": nama,
        "state": state
    }), 200 if telegram_terkirim else 502
